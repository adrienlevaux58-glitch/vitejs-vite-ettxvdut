...
function PlanButton({ recipe, plan, setPlan }: { recipe: Recipe; plan: MealPlan; setPlan: (p: MealPlan) => void }) {
  ...
  return (
    <div style={{ position: "relative" as const }}>
      <button
        onClick={(e) => { e.stopPropagation(); setOpen(!open); setSaved(false); }}
        style={{ padding: "7px 16px", borderRadius: 999, border: `1.5px solid ${saved ? "#16a34a" : "#e5e5e5"}`, background: saved ? "#f0fdf4" : "#fff", color: saved ? "#16a34a" : "#374151", fontSize: 13, cursor: "pointer", fontWeight: 500, display: "flex", alignItems: "center", gap: 6 }}
      >
      ...
      {open && !saved && (
        <div onClick={(e) => e.stopPropagation()} style={{ position: "absolute" as const, bottom: "calc(100% + 8px)", left: 0, background: "#fff", border: "1px solid #f1f0ee", borderRadius: 14, padding: "1rem", boxShadow: "0 8px 32px rgba(0,0,0,0.12)", zIndex: 50, minWidth: 240 }}>
          ...
          <button onClick={save} style={{ width: "100%", height: 36, borderRadius: 10, border: "none", background: "#C97D4E", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
            Confirmer
          </button>
 
          <button onClick={(e) => { e.stopPropagation(); setOpen(false); }} style={{ width: "100%", height: 30, borderRadius: 10, border: "none", background: "transparent", color: "#9ca3af", fontSize: 12, cursor: "pointer", marginTop: 4 }}>
            Annuler
          </button>
        </div>
      )}
...
// ---- RECIPE CARD ----
function RecipeCard({ recipe, index, highlight = false, plan, setPlan }: { recipe: Recipe; index: number; highlight?: boolean; plan?: MealPlan; setPlan?: (p: MealPlan) => void }) {
...
// ---- MEAL PLANNER ----
function MealPlanner({ plan, setPlan }: { plan: MealPlan; setPlan: (p: MealPlan) => void }) {
  ...
  return (
    <div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <div style={{ fontSize: 13, color: "#6b7280" }}>
              {filledCount === 0 ? "Ajoute des recettes depuis la recherche via 📅 Planifier" : `${filledCount} repas planifié${filledCount > 1 ? "s" : ""} sur 21`}
            </div>
          </div>
          {filledCount > 0 && (
            <button onClick={clearPlan} style={{ padding: "6px 14px", borderRadius: 999, border: "1.5px solid #fee2e2", background: "#fff8f6", color: "#dc2626", fontSize: 12, cursor: "pointer", fontWeight: 500 }}>
              Vider
            </button>
          )}
        </div>
        ...
        {filledCount > 0 && (
          <div style={{ marginTop: "1rem" }}>
            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#9ca3af", marginBottom: 4 }}>
              ...
              <span>{Math.round((filledCount / 21) * 100)}%</span>
            </div>
                ...
                return (
                  <div key={meal} style={{ borderRight: i < 2 ? "1px solid #f8f6f3" : "none", padding: "0.9rem 1rem", minHeight: 80 }}>
                    ...
                    {slot ? (
                      <div>
                        <div
                          onClick={() => { setSelectedSlot({ day, meal }); setMealRecipe(slot.recipe); setMealLoading(false); }}
                          style={{ fontSize: 12, color: "#374151", fontWeight: 500, lineHeight: 1.4, cursor: "pointer", marginBottom: 6 }}
                        >
                          {slot.recipe.titre}
                        </div>
      ...
      <div style={{ marginTop: "1rem", padding: "0.75rem 1rem", background: "#fdf9f6", borderRadius: 12, fontSize: 12, color: "#9ca3af", textAlign: "center" }}>
        💡 Ajoute tes recettes via <strong>📅 Planifier</strong> dans la recherche — ou clique <strong>+ Suggérer</strong> pour laisser l'IA proposer
      </div>
        ...
        <div style={{ position: "fixed" as const, inset: 0, background: "rgba(0,0,0,0.4)", zIndex: 100, display: "flex", alignItems: "flex-end", justifyContent: "center" }} onClick={() => { setSelectedSlot(null); setMealRecipe(null); }}>
          <div style={{ background: "#f9f7f4", borderRadius: "20px 20px 0 0", width: "100%", maxWidth: 640, maxHeight: "85vh", overflowY: "auto" as const }} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ padding: "1.25rem 1.5rem" }}>
              {/* Slot vide — proposer une suggestion IA */}
              ...
              {mealLoading && (
                <div style={{ textAlign: "center", padding: "2rem" }}>
                  ...
                  <div style={{ fontSize: 14, color: "#6b7280" }}>L'IA prépare une suggestion…</div>
                </div>
              )}
              ...
              {mealRecipe && !mealLoading && (
                <>
                  ...
                  {/* Si c'est une suggestion IA (case vide), proposer de l'ajouter */}
    ...
    <div style={{ minHeight: "100vh", background: "#f9f7f4", fontFamily: "system-ui, sans-serif" }}>
      ...
      <div style={{ background: "#fff", padding: "3rem 2rem 2.5rem", borderBottom: "1px solid #f1f0ee" }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <h1 style={{ fontSize: 32, fontWeight: 700, color: "#1a1a1a", marginBottom: 8, fontFamily: "'Georgia', serif", letterSpacing: "-0.5px", lineHeight: 1.2 }}>Qu'est-ce qu'on cuisine aujourd'hui ?</h1>
          <p style={{ fontSize: 15, color: "#6b7280", marginBottom: "2rem", lineHeight: 1.6 }}>Entre un ingrédient, plusieurs, ou un type comme "protéines" — PetitChef génère 3 recettes personnalisées en quelques secondes.</p>
          ...
          <div style={{ display: "flex", gap: 10, marginBottom: "1rem" }}>
            <div style={{ flex: 1, position: "relative" as const }}>
              ...
              <input value={query} onChange={(e) => setQuery(e.target.value)} onKeyDown={(e) => e.key === "Enter" && generate()} placeholder="poulet, riz, citron… ou 'végétarien rapide'" style={{ width: "100%", height: 52, padding: "0 18px 0 46px", borderRadius: 14, border: "1.5px solid #e5e5e5", fontSize: 15, background: "#fafaf9", color: "#1a1a1a", outline: "none", boxSizing: "border-box" as const }} onFocus={(e) => e.target.style.borderColor = "#C97D4E"} onBlur={(e) => e.target.style.borderColor = "#e5e5e5"} />
            </div>
            <button onClick={() => generate()} disabled={loading} style={{ height: 52, padding: "0 28px", borderRadius: 14, border: "none", background: loading ? "#e5a07a" : "#C97D4E", color: "#fff", fontSize: 15, fontWeight: 600, cursor: "pointer", whiteSpace: "nowrap" as const }}>{loading ? "…" : "Trouver"}</button>
          </div>
      ...
      <div style={{ maxWidth: 640, margin: "0 auto", padding: "2rem 1.5rem" }}>
        <div style={{ display: "flex", borderBottom: "1px solid #e5e5e5", marginBottom: "1.5rem" }}>
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)} style={{ flex: 1, padding: "10px 6px", fontSize: 11, fontWeight: tab === t.id ? 600 : 400, color: tab === t.id ? "#C97D4E" : "#6b7280", background: "none", border: "none", borderBottom: `2px solid ${tab === t.id ? "#C97D4E" : "transparent"}`, marginBottom: -1, cursor: "pointer" }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab === "recettes" && (
          <>
            ...
            {!loading && !searched && <div style={{ textAlign: "center", padding: "5rem 1rem" }}><div style={{ fontSize: 56, marginBottom: 20 }}>👨‍🍳</div><div style={{ fontSize: 18, fontWeight: 600, color: "#1a1a1a", marginBottom: 8, fontFamily: "'Georgia', serif" }}>Bienvenue sur PetitChef</div><div style={{ fontSize: 14, color: "#9ca3af", lineHeight: 1.6 }}>Entre un ingrédient ou une idée ci-dessus<br />pour découvrir vos recettes</div></div>}
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              {recipes.map((r, i) => <RecipeCard key={i} recipe={r} index={i} plan={plan} setPlan={setPlan} />)}
            </div>
          </>
        )}
 
        {tab === "planning" && <MealPlanner plan={plan} setPlan={setPlan} />}
        ...
        {tab === "favoris" && (
          <div>
            {favorites.length === 0 ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>{favorites.map((r, i) => <RecipeCard key={i} recipe={r} index={i} plan={plan} setPlan={setPlan} />)}</div>
            )}
          </div>
        )}
